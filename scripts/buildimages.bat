...
echo "building image docker-node-mongo-starter"
pushd source\backend
%CONTAINER_RUNTIME% build -f Dockerfile -t docker-node-mongo-starter .
popd

echo "building image my-app"
pushd source\frontend
%CONTAINER_RUNTIME% build -f Dockerfile -t my-app .
popd
