...
echo "building and pushing image docker-node-mongo-starter"
pushd source\backend
IF  "%CONTAINER_RUNTIME%" == "docker" (
    docker buildx build --platform ${PLATFORMS} -f Dockerfile  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/docker-node-mongo-starter . 
) ELSE ( 
    podman manifest create ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/docker-node-mongo-starter
    podman build --platform ${PLATFORMS} -f Dockerfile --manifest ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/docker-node-mongo-starter .
    podman manifest push ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/docker-node-mongo-starter
)
popd
echo "building and pushing image my-app"
pushd source\frontend
IF  "%CONTAINER_RUNTIME%" == "docker" (
    docker buildx build --platform ${PLATFORMS} -f Dockerfile  --push --tag ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/my-app . 
) ELSE ( 
    podman manifest create ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/my-app
    podman build --platform ${PLATFORMS} -f Dockerfile --manifest ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/my-app .
    podman manifest push ${REGISTRY_URL}/${REGISTRY_NAMESPACE}/my-app
)
popd
